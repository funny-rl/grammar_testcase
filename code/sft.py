...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        unflatten_mask = batch.pop("loss_mask")[:, :-1].cuda() # torch.Size([2, 2699])
            
        loss_mask = unflatten_mask.reshape(-1) # torch.Size([5398])
        ...
        with context, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            if not use_sp:
                ...
                labels = input_ids[:, 1:].contiguous() # torch.Size([2, 2699])
                output = self.fsdp_model(input_ids=input_ids, attention_mask=attention_mask, position_ids=position_ids, use_cache=False)
                logits = output.logits # torch.Size([2, 2700, 152064])
                
                shift_logits = logits[..., :-1, :].contiguous() # torch.Size([2, 2699, 152064])
                shift_labels = labels.contiguous() # torch.Size([2, 2699])
                
                if self.device_mesh.get_rank() == 0:
                    decoded_tokens = self.tokenizer.decode(shift_labels[0], skip_special_tokens=True)
                    print(decoded_tokens)
                    print(f"\n{'=' * 100}\n")
                torch.distributed.barrier()  # Ensure all ranks are synchronized before computing loss
                ...
                arg_token = logits[:, :-1, :].argmax(dim = -1) * unflatten_mask
                masked_labels = labels * unflatten_mask
                sentence_accuracy = (arg_token == masked_labels).all(dim=1).float().mean()
                token_accuracy = ((arg_token == masked_labels) * unflatten_mask).sum() / unflatten_mask.sum()
            ...
            return loss, sentence_accuracy, token_accuracy
    ...
    def training_step(self, batch: TensorDict):
        ...
        for micro_batch in micro_batches:
            loss, sent_acc, token_acc = self._compute_loss_and_backward(batch=micro_batch)
            loss /= n_micro_batches
            sent_acc /= n_micro_batches
            token_acc /= n_micro_batches

            step_loss += loss.item()
            step_sent_acc += sent_acc.item()
            step_token_acc += token_acc.item()
        ...
        step_sent_acc = torch.tensor(step_sent_acc).cuda()
        step_token_acc = torch.tensor(step_token_acc).cuda()
        torch.distributed.all_reduce(step_loss, op=torch.distributed.ReduceOp.AVG)
        torch.distributed.all_reduce(step_sent_acc, op=torch.distributed.ReduceOp.AVG)
        torch.distributed.all_reduce(step_token_acc, op=torch.distributed.ReduceOp.AVG)
        return {
            "train/loss": step_loss.detach().item(), 
            "train/lr(1e-3)": lr * 1e3,
            "train/sent_accuracy": step_sent_acc.detach().item(),
            "train/token_accuracy": step_token_acc.detach().item(),
        }

    def validation_step(self, batch: TensorDict):
        self.fsdp_model.eval()
        with torch.no_grad():
            loss, sent_acc, token_acc = self._compute_loss_and_backward(batch, do_backward=False)
            torch.distributed.all_reduce(loss, op=torch.distributed.ReduceOp.AVG)
            torch.distributed.all_reduce(sent_acc, op=torch.distributed.ReduceOp.AVG)
            torch.distributed.all_reduce(token_acc, op=torch.distributed.ReduceOp.AVG)
        return loss, sent_acc, token_acc

    def save_checkpoint(self, step):
        
        local_dir = self.config.trainer.default_local_dir
        local_path = os.path.join(local_dir, f'global_step_{step}')
        if self.device_mesh.get_rank() == 0:
            if os.path.exists(local_dir):
                subdirs = [name for name in os.listdir(local_dir) if os.path.isdir(os.path.join(local_dir, name))]
                for name in subdirs:
        ...
        if self.config.model.get("lora_rank", 0) > 0:
            with warnings.catch_warnings():
                ...
                with FSDP.state_dict_type(
                    ...
                    peft_model_module = self.fsdp_model.module
                    peft_model_module.save_pretrained(local_path)
                    
                    if self.device_mesh.get_rank() == 0:    
                        print(f"✅✅✅ SAVE SUCCESS: PEFT model saved to {local_path} ✅✅✅")
        else:           
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                with FSDP.state_dict_type(
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for data in self.val_dataloader:
                ...
                val_loss, sent_acc, token_acc = self.validation_step(data)
            ...
            if rank == 0:
                ...
                metric = {
                    ...
                    'val/sent_accuracy': sent_acc.detach().item(),
                    'val/token_accuracy': token_acc.detach().item(),
                }
                tracking.log(data=metric, step=global_step)
            torch.distributed.barrier()
...
def main(config):
    ...
    if rank == 0:
        load_dataset(
            ...
            model_name= config.model.partial_pretrain,
