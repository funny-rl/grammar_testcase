...
def main_task(config):
    ...
    answer_list = [[] for _ in range(config.data.n_samples)]
    ...
    for batch_idx in range(num_batch):
        ...
        for n_sample in range(config.data.n_samples):
            ...
            for i in range(len(output)): # len(output) -> batch_size x n
                ...
                try:
                    print(f"Response {i}: {ast.literal_eval(grammar_str)}")
                    grammar_str = ast.literal_eval(grammar_str)
                except Exception as e:
                    print(f"Error parsing grammar string: {grammar_str}, Error: {e}")
                grammar_texts.append(grammar_str)

            answer_list[n_sample].extend(grammar_texts)

    answer_list = np.array(answer_list, dtype=object)
    answer_list = np.transpose(answer_list, axes=(1, 0)).tolist()
    
    dataset["grammar"] = answer_list
