...
class CountingContextFreeGrammar:
    ...
    def generate_with_assignment(self, assignment: dict[str, int]) -> str:
        """Generate a testcase using a fixed variable assignment."""
        string = ""
        ...
        while derivation_queue:
            ...
            if token_type == TokenType.TERMINAL:
                ...
                string += self._sample_terminal(terminal, assignment, degree=0)
                ...
                value = assignment.get(str(assignment_form_variable), None)
                if value is None:
                    raise ValueError(f"No value provided for variable: {assignment_form_variable}")
    ...
    def generate(self, *, degree: int = 0) -> str:
        ...
        print("Final assignment of variables:")
    ...
    def _sample_variable(
        ...
        *,
        ...
        """Sample a value of a constraint photon variable"""
    ...
    def _tokenize(string: str) -> Token:
        if string in {
            ...
            # Validate variable name: allow [name] or bare name, where name is alphanumeric + underscore
            if not re.match(r"^\[?[a-zA-Z0-9_]+\]?$", string):
                raise InvalidProductionError(f"Invalid variable name: {string}")
    ...
    def _sample_terminal(
        ...
        # Check for unsupported regex quantifiers (+ or *)
        if terminal.endswith('+') or terminal.endswith('*'):
            raise ValueError(f"Unsupported regex quantifier in terminal: {terminal}")
        ...
        assignment[Variable(str(terminal))] = terminal_len
